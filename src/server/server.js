...
// Initialiser la table pour les scores
db.serialize(() => {
...
const tickGame = () => {
    ...
    map.players.handleCollisions(function (gotEaten, eater) {
        ...
        if (playerDied) {
			...
			// Recalculer la masse totale pour le joueur mangeur
            const finalMassTotal = playerWhoAte.cells.reduce((total, cell) => total + cell.mass, 0);
			
			saveScore(playerWhoAte.name, finalMassTotal); // Enregistrer le score pour le mangeur
...
function saveScore(playerName, score) {
    ...
    db.run(`INSERT INTO scores (name, score, timestamp) VALUES (?, ?, ?)`, [playerName, score, timestamp], (err) => {
        if (err) {
...
function getTopScores(period, callback) {
    let query;
    const now = Date.now();
    if (period === 'daily') {
        query = `SELECT name, score FROM scores WHERE timestamp >= ? ORDER BY score DESC LIMIT 3`;
        db.all(query, [now - 24 * 60 * 60 * 1000], callback);
    } else if (period === 'weekly') {
        query = `SELECT name, score FROM scores WHERE timestamp >= ? ORDER BY score DESC LIMIT 3`;
        db.all(query, [now - 7 * 24 * 60 * 60 * 1000], callback);
    } else if (period === 'monthly') {
        query = `SELECT name, score FROM scores WHERE timestamp >= ? ORDER BY score DESC LIMIT 3`;
        db.all(query, [now - 30 * 24 * 60 * 60 * 1000], callback);
    }
}

app.get('/api/top-scores', (req, res) => {
    const period = req.query.period;
    getTopScores(period, (err, rows) => {
        if (err) {
